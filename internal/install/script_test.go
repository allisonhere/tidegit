...
func TestInstallScriptDefaultsToUserLocalBinWithoutSudo(t *testing.T) {
	tmp := t.TempDir()
	archive := writeInstallTestArchive(t, tmp, "tidegit test-version", 0)
	bin := filepath.Join(tmp, "fakebin")
	writeInstallTestTools(t, bin, archive, "")

	cmd := exec.Command("sh", installScriptPath(t))
	...
	installed := filepath.Join(tmp, ".local", "bin", "tidegit")
...
func TestInstallScriptRemovesWritableStaleBinaryEarlierOnPath(t *testing.T) {
	...
	stale := filepath.Join(staleBin, "tidegit")
...
func TestInstallScriptLeavesExistingBinaryWhenDownloadedBinaryFailsVersionCheck(t *testing.T) {
	...
	installed := filepath.Join(installDir, "tidegit")
...
// installScriptPath locates install.sh at the repository root; the tests run
// with this package's directory as their working directory.
...
func writeInstallTestArchive(t *testing.T, dir, version string, exitCode int) string {
	t.Helper()
	path := filepath.Join(dir, "tidegit-linux-x86_64.tar.gz")
	var buf bytes.Buffer
	gz := gzip.NewWriter(&buf)
	tw := tar.NewWriter(gz)
	body := "#!/bin/sh\nif [ \"${1:-}\" = \"--version\" ]; then echo " + shellQuote(version) + "; exit " + strconv.Itoa(exitCode) + "; fi\n"
	if err := tw.WriteHeader(&tar.Header{Name: "tidegit-linux-x86_64", Mode: 0o755, Size: int64(len(body))}); err != nil {
		t.Fatal(err)
	}
...
func writeInstallTestTools(t *testing.T, dir, archive, installScript string) {
	...
	// URL-aware fake curl: fail the checksums fetch (script then soft-continues),
	// copy the archive for the asset fetch.
	writeExecutable(t, filepath.Join(dir, "curl"), "#!/bin/sh\nout=''\nfor a in \"$@\"; do case \"$a\" in *checksums.txt) exit 22;; esac; done\nwhile [ \"$#\" -gt 0 ]; do\n  if [ \"$1\" = '-o' ]; then shift; out=\"$1\"; fi\n  shift || break\ndone\ncp "+shellQuote(archive)+" \"$out\"\n")
...
func assertNoInstallTemps(t *testing.T, dir string) {
	t.Helper()
	matches, err := filepath.Glob(filepath.Join(dir, ".tidegit.tmp.*"))
	if err != nil {
		t.Fatal(err)
	}
	if len(matches) > 0 {
